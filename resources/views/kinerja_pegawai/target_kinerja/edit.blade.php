...
@section('page-name', 'Edit KM & Sasaran Mutu')

@section('content-base')
<div class="mb-4">
    <p class="text-sm text-gray-500 italic">Perbarui informasi indikator KPI global.</p>
</div>
<div x-data="satuanCrud()" x-init="fetchSatuans()" class="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 bg-white rounded-lg shadow-sm border border-gray-100">
    <form action="{{ route('manage.target-kinerja.update', $targetKinerja->id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama_kpi" value="{{ old('nama_kpi', $targetKinerja->nama_kpi) }}" required
                ...
                <div>
                    ...
                    <select name="responsibility_id" required class="w-full text-sm text-gray-900 border-gray-300 rounded-md shadow-sm focus:ring-blue-500">
                        ...
                        @foreach(\App\Models\Unit::orderBy('nama_unit')->get() as $unit)
                            <option value="{{ $unit->id }}" {{ old('responsibility_id', $targetKinerja->responsibility_id) == $unit->id ? 'selected' : '' }}>{{ $unit->nama_unit }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="satuan" required class="w-full text-sm text-gray-900 border-gray-300 rounded-md shadow-sm focus:ring-blue-500">
                        ...
                        <template x-for="s in satuans" :key="s.id">
                            <option :value="s.nama" x-text="s.nama" :selected="s.nama == '{{ old('satuan', $targetKinerja->satuan) }}'"></option>
                        </template>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="jenis" required class="w-full text-sm text-gray-900 border-gray-300 rounded-md shadow-sm focus:ring-blue-500">
                        ...
                        <option value="Kontrak Manajemen" {{ old('jenis', $targetKinerja->jenis) == 'Kontrak Manajemen' ? 'selected' : '' }}>Kontrak Manajemen (KM)</option>
                        <option value="Sasaran Mutu" {{ old('jenis', $targetKinerja->jenis) == 'Sasaran Mutu' ? 'selected' : '' }}>Sasaran Mutu (SM)</option>
                    </select>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-600 mb-2">Tahun Anggaran</label>
                    <input type="number" name="tahun" value="{{ old('tahun', $targetKinerja->tahun) }}" required
        ...
        <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
            ...
            <p class="text-xs text-gray-400 italic mb-6">Tentukan target pencapaian dan bobot nilai untuk setiap triwulan.</p>
                ...
                @foreach(['tw1' => 'Triwulan I', 'tw2' => 'Triwulan II', 'tw3' => 'Triwulan III', 'tw4' => 'Triwulan IV'] as $key => $label)
                    <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm space-y-4">
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="number" step="0.01" name="{{ $key }}_target" value="{{ old($key.'_target', $targetKinerja->{$key.'_target'} ?? 0) }}"
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="number" step="0.01" name="{{ $key }}_bobot" value="{{ old($key.'_bobot', $targetKinerja->{$key.'_bobot'} ?? 0) }}"
                                class="w-full text-sm text-gray-900 border-gray-300 rounded-md focus:ring-blue-500">
                        </div>
                    </div>
                @endforeach
        ...
        <div class="space-y-6">
            <div>
                ...
                <textarea name="keterangan" rows="4"
                    ...
                    placeholder="Tambahkan informasi detail indikator ini...">{{ old('keterangan', $targetKinerja->keterangan) }}</textarea>
            </div>

            <div class="flex items-center">
                <input type="checkbox" name="is_active" value="1" id="is_active" {{ $targetKinerja->is_active ? 'checked' : '' }}
                    class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                <label for="is_active" class="ml-2 block text-sm font-medium text-gray-700">
                    Indikator KPI ini aktif dan dapat digunakan
                </label>
            </div>
        </div>
